...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-6-implement-test-nodes.md`
  summary: Fix two real, pre-existing app bugs found during live Test-node verification -- (1) the root route `/` no longer returns 200 (redirects, breaking ExampleTest and AuthenticationTest::users_can_logout), a regression from the "Refresh finance UI" commit; (2) ProfileController's account-deletion flow calls `Auth::logout()` on a guard that doesn't implement it, breaking ProfileTest::user_can_delete_their_account.
  evidence: Not fixed by this story deliberately -- these are Joseph's app bugs, not infrastructure gaps. (2) specifically touches auth code that FR-5's denylist forbids the autonomous pipeline from ever modifying, so it must be fixed by Joseph directly, not by a future autonomous Ticket. Both currently make the baseline test suite fail 3/25 regardless of any future code change -- worth fixing soon so real Tickets aren't penalized by unrelated pre-existing failures.
