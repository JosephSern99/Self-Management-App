...
- ~~**Two pre-existing app bugs found during Story 1.6 live verification**~~ — Resolved 2026-09-02, by Joseph/Claude directly (not the autonomous pipeline, since one touches denylisted auth territory). Root cause of the `Auth::logout()` crash: `routes/web.php` guarded profile routes with `auth:sanctum` instead of `auth`, switching the request's default guard to a `RequestGuard` with no `logout()`. Fixed by giving profile routes their own `auth` group; finance/payment/transaction routes deliberately left on `auth:sanctum` (not verified safe to change). Two stale test assertions also fixed. Baseline is now 25/25 passing.
